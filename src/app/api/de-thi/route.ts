...
import { monCoKiemTraDinhKy } from "@/lib/subjects";
...
function buildPrompt(monHoc: string, khoiLop: string, tenBai: string, soCauhoi: number) {
  return `Bạn là chuyên gia ra đề kiểm tra định kỳ Tiểu học Việt Nam, bám sát chương trình GDPT 2018 và Thông tư 27/2020/TT-BGDĐT về đánh giá học sinh tiểu học. Câu hỏi phải phù hợp lứa tuổi học sinh lớp ${khoiLop}: ngôn ngữ đơn giản, hình ảnh minh hoạ nếu cần mô tả, tránh bẫy đánh đố.

Hãy soạn một đề kiểm tra định kỳ cho:
- Môn học: ${monHoc}
- Khối lớp: ${khoiLop}
- Chủ đề/bài: ${tenBai}
...
- Độ khó tăng dần theo 4 mức: nhận biết, thông hiểu, vận dụng, vận dụng cao — phù hợp trình độ học sinh khối lớp trên.
...
export async function POST(req: NextRequest) {
  ...
  // Thông tư 27/2020/TT-BGDĐT: lớp 1-2 chỉ có bài kiểm tra định kỳ cho Toán
  // và Tiếng Việt — các môn khác chỉ đánh giá bằng nhận xét, không có đề thi.
  if (!monCoKiemTraDinhKy(khoiLop).includes(monHoc)) {
    return NextResponse.json(
      { error: `Lớp ${khoiLop} không có bài kiểm tra định kỳ cho môn ${monHoc} (chỉ đánh giá bằng nhận xét theo Thông tư 27)` },
      { status: 400 }
    );
  }
