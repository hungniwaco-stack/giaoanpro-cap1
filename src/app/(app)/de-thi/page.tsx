...
import { KHOI_LOP, monCoKiemTraDinhKy } from "@/lib/subjects";
...
export default function DeThiPage() {
  ...
  const [monHoc, setMonHoc] = useState(monCoKiemTraDinhKy(KHOI_LOP[0])[0]);
  const [tenBai, setTenBai] = useState("");
  const [thoiGianLamBai, setThoiGianLamBai] = useState("40 phút");
  ...
  function handleKhoiChange(k: string) {
    setKhoiLop(k);
    const monList = monCoKiemTraDinhKy(k);
    if (!monList.includes(monHoc)) setMonHoc(monList[0]);
  }
  ...
  return (
    <main className="px-6 py-8 sm:px-10 sm:py-10">
      ...
      <p className="mt-1 text-sm text-ink-muted">
        Ra đề kiểm tra định kỳ trộn trắc nghiệm + tự luận, kèm đáp án, xuất Word & PPT ngay.
      </p>
          ...
          <div className="relative p-6 pl-16">
            ...
            <div className="mt-4">
              ...
              <select
                ...
                className="mt-1 w-full rounded-lg border border-ink/15 bg-white px-3 py-2 text-ink outline-none focus:border-pine"
              >
                {monCoKiemTraDinhKy(khoiLop).map((m) => (
                  <option key={m} value={m}>{m}</option>
                ))}
              </select>
              {(khoiLop === "1" || khoiLop === "2") && (
                <p className="mt-1 text-xs text-ink-muted/70">
                  Lớp 1-2 chỉ có bài kiểm tra định kỳ cho Toán và Tiếng Việt (Thông tư 27), các môn khác đánh giá bằng nhận xét.
                </p>
              )}
            </div>
