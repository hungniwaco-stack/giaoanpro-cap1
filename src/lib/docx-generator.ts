...
export function generateLessonPlanDocx(plan: LessonPlan): Document {
  const activityRows = plan.hoatDong.flatMap((hd) => [
    ...
    new TableRow({ children: [labeledCell("Mục tiêu", 25), textCell(hd.mucTieu, 75)] }),
    new TableRow({ children: [labeledCell("Cách thực hiện", 25), textCell(hd.cachThucHien, 75)] }),
  ]);
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({ heading: HeadingLevel.HEADING_2, text: "I. Yêu cầu cần đạt" }),
          ...bulletList(plan.yeuCauCanDat),

          new Paragraph({ heading: HeadingLevel.HEADING_2, text: "II. Đồ dùng dạy học" }),
          ...bulletList(plan.doDungDayHoc),

          new Paragraph({ heading: HeadingLevel.HEADING_2, text: "III. Các hoạt động dạy học chủ yếu" }),
          new Table({
