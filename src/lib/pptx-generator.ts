...
export function generateLessonPlanPptx(plan: LessonPlan): pptxgen {
  ...
  addBulletSlide(pptx, "Yêu cầu cần đạt", plan.yeuCauCanDat);
  addBulletSlide(pptx, "Đồ dùng dạy học", plan.doDungDayHoc);

  for (const hd of plan.hoatDong) {
    addBulletSlide(pptx, hd.ten, splitSentences(hd.cachThucHien));
  }
